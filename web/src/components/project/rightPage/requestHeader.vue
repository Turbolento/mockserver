<script>
  export default {
    props: ['requestHeaders','responseBodys'],
    methods: {
      removeRequestHeader: function (num) {
        var child = document.getElementById("requestHeader" + num);
        child.parentNode.removeChild(child);
        var arr = this.requestHeaders
...
<template>
  <div class="requestHeader-list" id="requestHeaderTable">
    <div
      class="requestHeader-item"
                      ...
                      <div>
                        <ul class="cb">
                          <li class="col-sm-1"><i class="fa fa-remove" style="margin: 0 4px;cursor:pointer;"
                                                  v-on:click="removeRequestHeader(requestHeader.id)"></i>
                            <i class="fa fa-reorder" draggable="true" style="margin: 0 4px;cursor:move;"></i></li>
                          <li class="col-sm-3 input">
                            <input type="text" class="text name" v-model="requestHeader.name"></li>
                          <li class="col-sm-2">
                            <select class="uk-select"
                                    style="position: absolute;top:0px;left:0px;height:38px;display: inline-block" v-model="requestHeader.match">
                              <option value="0">等于</option>
                              <option value="1">包含</option>
                              <option value="2">大于</option>
                              <option value="3">小于</option>
                              <option value="4">开始于</option>
                              <option value="5">结束于</option>
                              <option value="6">任意值</option>
                            </select>
                          </li>
                          <li class="col-sm-2 input"><input type="text" draggable="false" class="text" v-model="requestHeader.value"></li>
                          <li class="col-sm-4 input">
                            <select class="uk-select" :style="{'color':requestHeader.response?'':'red'}"
                                    style="position: absolute;top:0px;left:0px;height:38px;display: inline-block;" v-model="requestHeader.response">
                              <option value="">请选择(响应数据为空则先添加)</option>
                              <option v-for="responseBody in responseBodys" :value="responseBody.name">{{responseBody.name}}</option>
                            </select>
                          </li>
                        </ul>
                      </div>
